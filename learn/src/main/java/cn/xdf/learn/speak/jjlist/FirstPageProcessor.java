...
public class FirstPageProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		List<String> category = page.getHtml().$("div.list-wrap").$("div.list-wrap-max").xpath("//p[2]/a[1]/@href").all();
//		String Writing = page.getHtml().$("div.list-wrap").xpath("//div[1]//p[1]/text()").toString();
//		List<String> jjlists = page.getHtml().$("div.list-wrap").$("div.list-wrap-max").all();
//		System.out.println(jjlists.size());
//		System.out.println(category);
//		System.out.println(Writing);
//		for (String string : category) {
////			System.out.println("链接："+string);
//			String jjlist = page.getHtml().$("div.list-wrap").$("div.list-wrap-max").xpath("//p[1]/text()").toString();
//			System.out.println(jjlist);
//			Spider.create(new SecPageProcessor()).addUrl(string).run();
//		}
		for (int i = 1; i <= category.size(); i++) {
			String jjlist = page.getHtml().$("div.list-wrap").xpath("//div["+i+"]//p[1]/text()").toString();
			System.out.println("===========================满分口语机经 ==============================");
			System.out.println(jjlist);
			Spider.create(new SecPageProcessor()).addUrl(category.get(i-1)).run();
		}
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 112/10+1; i++) {
			Spider.create(new FirstPageProcessor())
			.addUrl("http://toefl.kaomanfen.com/speaking/jjlist?jjid=639&p="+i).run();
		}
